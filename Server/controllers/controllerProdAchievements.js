...
module.exports = {
    ...
    try {
      ...
      if (req.query) {
        ...
        let mapPrbAnalysis = await problemHistoryData.map(async (item) => {
          ...
          const prbAnalysis = await cmdMultipleQuery(
            `SELECT json_string FROM o_analisys WHERE id_problem = ${prob_id} LIMIT 1`
          );
          prbAnalysis.length > 0
  ...
  addProblem: (req, res) => {
    ...
    dbSingleQuery(qCheckProblem)
      .then((result) => {
        console.log(
          `SELECT fid FROM tb_status WHERE fid = ${fmc_id} AND fstatus = '0'`
        );
        console.log(result);
        if (result.length > 0) {
          dbSingleQuery(qAddProb).then((results) => {
  ...
  editProblem: async (req, res) => {
    let pathFimgProblem = req.files.fimage_problem
      ...
      : null;
    let pathStdImg = req.files.std_img
      ? `${req.files.std_img[0].destination}${req.files.std_img[0].filename}`
      : null;
    let pathActImg = req.files.act_img
      ? `${req.files.act_img[0].destination}${req.files.act_img[0].filename}`
      : null;
    let pathWhyImg = req.files.why1_img
      ? `${req.files.why1_img[0].destination}${req.files.why1_img[0].filename}`
      : null;

    let pathFimgProblem2 = req.files.fimage2_problem
      ? `${req.files.fimage2_problem[0].destination}${req.files.fimage2_problem[0].filename}`
      : null;
    let pathStdImg2 = req.files.std2_img
      ? `${req.files.std2_img[0].destination}${req.files.std2_img[0].filename}`
      : null;
    let pathActImg2 = req.files.act2_img
      ? `${req.files.act2_img[0].destination}${req.files.act2_img[0].filename}`
      : null;
    let pathWhyImg2 = req.files.why2_img
      ? `${req.files.why2_img[0].destination}${req.files.why2_img[0].filename}`
      : null;
    let pathWhyImg12 = req.files.why12_img
      ? `${req.files.why12_img[0].destination}${req.files.why12_img[0].filename}`
      : null;
    let pathWhyImg22 = req.files.why22_img
      ? `${req.files.why22_img[0].destination}${req.files.why22_img[0].filename}`
      : null;
    ...
    for (const key in req.body) {
      ...
      if (key == "furaian_kejadian_general")
        uraian.push({
          desc_name: req.body["furaian_kejadian_general"],
          type_uraian: "general",
        });
      if (key == "furaian_kejadian_standard")
        uraian.push({
          desc_name: req.body["furaian_kejadian_standard"],
          type_uraian: "standard",
        });
      if (key == "furaian_kejadian_actual")
        uraian.push({
          desc_name: req.body["furaian_kejadian_actual"],
          type_uraian: "actual",
        });
    }
    console.log(uraian);
    if (uraian.length > 0) {
      ...
      for (let i = 0; i < uraian.length; i++) {
        ...
        await cmdMultipleQuery(
          `SELECT id from tb_r_uraian where error_id = ${req.params.v_} AND type_uraian='${element.type_uraian}'`
        )
          .then((result) => {
            console.log(result);
            let ilusUraian = null;
            ...
            if (result.length > 0) {
              qUraianUpdate.push(
                `UPDATE tb_r_uraian SET desc_nm = '${element.desc_name}' where error_id = ${req.params.v_} AND type_uraian='${element.type_uraian}'`
              );
            } else {
              qUraianInsert.push(
                `INSERT INTO tb_r_uraian(error_id, desc_nm, type_uraian) VALUES (${req.params.v_}, '${element.desc_name}', '${element.type_uraian}')`
              );
            }
    ...
    for (key in req.body) {
      ...
      if (key == "fstart_time" || key == "fend_time") {
        ...
        key == "fiveWhyLhApprove" ||
        key == "fiveWhyShApprove" ||
        key == "cmLhApprove" ||
        key == "cmShApprove"
      ) {
        ...
        ((key == "why1_img" && delWhy1 == 1) ||
          (key == "why12_img" && delWhy12 == 1) ||
          (key == "why2_img" && delWhy2 == 1) ||
          (key == "why22_img" && delWhy22 == 1)) &&
        !req.body[key]
      ) {
        qEditProb += ` ${key}='${req.body[key]}'`;
      } else if (
    ...
    if (req.query.isFinished) {
      ...
      console.log(req.body);
      let arrMt =
        typeof req.body.foperator == "string"
          ? [req.body.foperator]
          : req.body.foperator;
      let arrQueryCek = [];
      let qResMt = "";
    ...
    if (containerQuery.length <= 1) {
      ...
      await cmdMultipleQuery(qUpdateColDash)
        .then(({ data }) => {
          console.log(data);
        })
        .catch((err) => {
      ...
      await cmdMultipleQuery(qCloseNotif)
        .then(async ({ data }) => {
