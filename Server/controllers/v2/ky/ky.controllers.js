  ...
  getKY: async (req, res) => {
    try {
      ...
      let containerKyQueries = await machinesData.map(async (item) => {
        return `SELECT *, IF(ilustration IS NOT NULL, CONCAT('http://mt-system.id:3100/image?path=', ilustration), null) as ilustration FROM tb_m_kymachine WHERE machine_id = ${item.machine_id}`;
      });
