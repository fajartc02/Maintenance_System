...
module.exports = {
        ...
        "YYYY-MM-DD"
                ...
                let q = `SELECT * FROM tb_r_floating_tip ${queryCondition} ORDER BY check_date ASC`;
                        ...
                        parameters.forEach((paramInfo) => {
                            ...
                            const paramData = {
                                ...
                                notes: records
                                    ...
                                    .map((item) => ({
                                        x: moment(item.check_date).add(7, "hours").unix() * 1000,
                                        y: item[paramInfo.key],
                                        marker: {
                                            size: 0,
                                        },
                                        label: {
        ...
        "YYYY-MM-DD"
                ...
                const responseData = await cmdMultipleQuery(q);
                if (responseData.length > 0) {
                    const remapData = await responseData.map((item) => {
                        item.check_date = moment(item.check_date).format(
                            "YYYY-MM-DD HH:mm:ss"
                        );
                        return item;
                    });
                    response.success(res, "get data", remapData);
                } else {
                    response.success(res, "get data", []);
                }
            } catch (error) {
        ...
        addTip: async(req, res) => {
            ...
            element
              ? `'${element}${
                  key == "check_date" ? ` ${moment().format("HH:mm:ss")}` : ""
                }'`
              : null
          }`
        );
